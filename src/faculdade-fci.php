...
$resultado = mysqli_query($conn, "SELECT faculdade_nomecurto, faculdade_nomelongo, descritivo, telefone, email, predio_numero, predio_nome, latitude, longitude from faculdades where id = '$pageid'") or die("Não foi possível executar a SQL: " . mysqli_error($conn));
if ($resultado) {
    while ($row = mysqli_fetch_array($resultado)) {
        ...
        echo "<p class='grey-text text-darken-3 lighten-3'>" . $row['descritivo'] . "</p></div></div><div class='section amber lighten-5'><div class='row container'>";
        ...
        echo "<p><a href='alteracao.php?pageid=$pageid&categoria=$categoria'>Sugerir alteração</a></p>" . "</div></div><iframe width='600' height='400' frameborder='0' style='border:0' src='https://www.google.com/maps/embed/v1/view?key=AIzaSyChUzQvO3rUvsvWQ57UxThFUY_ZrvumGeg&center=$row[latitude],$row[longitude]&zoom=21&maptype=satellite' allowfullscreen></iframe></div></main></div>";
