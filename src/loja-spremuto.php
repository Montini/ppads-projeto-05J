...
$resultado = mysqli_query($conn, "SELECT loja_nomecurto, loja_nomelongo, descritivo, telefone, email, predio_numero, predio_nome from restaurantes where id = '$pageid'") or die("Não foi possível executar a SQL: ".mysqli_error($conn));
if($resultado){
    while($row = mysqli_fetch_array($resultado) ){
        ...
        echo "<h2><a class='collection-item waves-effect black-text'>".$row["loja_nomelongo"]."</a></h2>";
        ...
        echo "<p><a href='alteracao.php?pageid=$pageid&categoria=$categoria'>Sugerir alteração</a></p>" . "</div></div></div></main></div>";
